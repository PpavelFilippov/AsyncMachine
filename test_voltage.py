...
def main() -> None:
    ...
    base_params = MachineParameters()
    solver_cfg = SolverConfig(dt_out=2e-4)
    output_root = os.path.join("output", "test_voltage")
    ...
    for slug, factor, label in voltage_cases:
        params = replace(base_params, Unom_line=base_params.Unom_line * factor)
        ...
        print(
            f"  CASE: {label} | "
            f"Unom_line={params.Unom_line:.1f} В, Um={params.Um:.1f} В (factor={factor:.2f})"
        )
        print("#" * 70)
        print(params.info())
    ...
    print(f"\n  Графики сохранены в: {output_root}/")
