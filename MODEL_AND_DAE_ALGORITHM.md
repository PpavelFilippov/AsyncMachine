...
3.  Сегмент 2 после `t_fault`: решается расширенная система через `DAESimulationBuilder` и `CircuitAssembler`.
...
-   Без КЗ: используется единый DAE путь через матричную сборку схемы.
-   С КЗ: используется единый DAE путь с расширением состояния и `CircuitAssembler`.
