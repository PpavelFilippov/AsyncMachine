"""
Compare MotorStepLoadScenario with ideal vs non-ideal (Thevenin) source.
...
class MotorStepLoadWithSourceScenario(MotorStepLoadScenario):
    """Motor-step scenario with injectable voltage source factory."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test MotorStepLoadScenario with ideal and Thevenin source."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("output") / "tests" / "motor_step_sources",
...
def main() -> None:
    ...
    scenario_ideal = MotorStepLoadWithSourceScenario(
    ...
    csv_path = output_dir / "motor_step_sources_metrics.csv"
    _save_csv(rows, csv_path)

    print("\nMotor-step source comparison:")
    for r in rows:
        print(
