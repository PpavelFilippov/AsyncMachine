...
from models.saturation import I_OE_TABLE, SaturationCharacteristic
...
def make_flat_saturation(params: MachineParameters) -> SaturationCharacteristic:
    """
    Saturation characteristic with K_sat == 1 for all operating points.

    This mirrors validation logic in test.py where nonlinear should match
    linear with respect to magnetizing inductance law.
    """
...
def run_and_plot_scenario(
    *,
    ...
    print("  Nonlinear model call is aligned with test.py (explicit saturation object).")
    print("  Using flat saturation (K_sat = 1) to keep Lm consistent across sweeps.")
